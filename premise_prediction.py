...
model.to(device)
model.load_state_dict(torch.load("trainmodel/.pth",map_location=device))
model.eval()
...
#val = val.drop(columns=['Premise']).rename(columns={'predictions':'Premise'})         
val = val.rename(columns={'predictions':'Premise'})                                             
#save model predictions to .tsv file
val.to_csv('file_tsv/.tsv', sep='\t', index=False)